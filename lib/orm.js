...
class ORM {
  ...
  normalizeModel(model, keepNull) {
    ...
    for (let key in model) {
      ...
      let checkedKey = this.options.useSnakeCase ? (0, _voca.snakeCase)(key) : key;
  ...
  _processLikeClause(mode, model, key, word, conditions, args, start, end) {
    ...
    if (Array.isArray(word)) {
      ...
      throw new Error(_util.default.format("bad %s condition, expecting string or array,found:%j, key = %s, model = %j", mode, word, key, model));
    }
  ...
  modelToConditions(model, keepNull) {
    ...
    for (let key in model) {
      let value = model[key]; // do not use isNaN on array
      // NOTE: NaN == null => false
      ...
      if (!Array.isArray(value)) {
        ...
        if (value.length === 0) {
          ...
          throw new Error(_util.default.format("empty array condition: key = %s, model = %j", key, model));
        }
  ...
  quoteName(name) {
    return (0, _mysql.escapeId)(name); // let split = name.split(".")
  ...
  _getUpdateSQL(model, where, options) {
    ...
    let {
      conditions,
      args
    } = this.modelToConditions(where);
    return {
      sql: "UPDATE ?? SET ? WHERE " + conditions.join(" AND "),
      args: [this.table, checkedModel, ...args]
    };
  }
  ...
  _getDeleteSQL(where) {
    ...
    return {
      sql: "DELETE FROM ?? WHERE " + conditions.join(" AND "),
      args: [this.table, ...args]
    };
  ...
  async update(model, where, options) {
    ...
    let {
      sql,
      args
    } = this._getUpdateSQL(model, where, options);
  ...
  async updateMany(data, options) {
    ...
    (0, _validate.validateArrayNotEmpty)({
      data
    });
    let sqlArgs = data.map(({
      data,
      where
    }) => this._getUpdateSQL(data, where, options));
    return Promise.all(sqlArgs.map(({
      sql,
      args
  ...
  async deleteWhere(where) {
    (0, _validate.validateObjectAtLeastOneKey)({
      where
    });
  ...
  async insertOrUpdateMany(tx, key, data, idField, ids, options) {
    ...
    if (ids.length < data.length) {
      throw new Error(_util.default.format("insertOrUpdateMany:ids.length=%s < data.length=%s, may not be enough!", ids.length, data.length));
    }
    ...
    const items = data.map((model, idx) => {
      ...
      const where = {};
      key.forEach(k => where[k] = model[k]); // conditions cannot be empty
    ...
    const result = items.map(async ({
      model,
      conditions,
      args
    }, idx) => {
      return await this._doInsertOrUpdate(tx, idField, model, conditions, args, ids[idx], createPreset, updatePreset, {
        keepNull
      });
    });
    ...
    if (!Array.isArray(where)) {
      ...
      if (checkedModel === undefined) {
        ...
        let {
          conditions,
          args
        } = this.modelToConditions(checkedModel);
