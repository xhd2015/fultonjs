...
async function doWithRepos(primaryRepo, branch, subRepos, callback) {
  ...
  try {
    ...
    const {
      dir: primaryDir,
      close: primaryClose
    } = await gitRepo.useBranch(branch);
    deferCloses.push(primaryClose);

    for (let subRepo of subRepos) {
      const md5 = shell.md5(subRepo);
      const subGitRepo = new git.GitRepo(`/tmp/grpc-repo/${md5}`, subRepo); // no branch specified, cannot apply these branch

      const {
        dir: subDir,
        close: subClose
      } = await subGitRepo.useBranch('master');
