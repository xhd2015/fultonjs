...
exports.ROOT_UNDERSCORE = exports.ROOT_PREFIX_UNDERSCORE = exports.ROOT_PREFIX = exports.ROOT_FIELDS = exports.ROOT = exports.IGNORE = void 0;
exports.getDisplaySchema = getDisplaySchema;
exports.inflateSchema = inflateSchema;
exports.mergeLaplaceFields = mergeLaplaceFields;
exports.mergeSchemaExtends = mergeSchemaExtends;
...
const ROOT = "$root";
exports.ROOT = ROOT;
const ROOT_UNDERSCORE = "_$root";
exports.ROOT_UNDERSCORE = ROOT_UNDERSCORE;
const ROOT_PREFIX = ROOT + ".";
exports.ROOT_PREFIX = ROOT_PREFIX;
const ROOT_PREFIX_UNDERSCORE = ROOT_UNDERSCORE + ".";
exports.ROOT_PREFIX_UNDERSCORE = ROOT_PREFIX_UNDERSCORE;
const IGNORE = "$ignore"; // just possible fields
...
const ROOT_FIELDS = {
  ...
  create_time: {},
  update_time: {},
  deleted: {// optional, only for mysql
  }
};
/**
 * 
...
exports.ROOT_FIELDS = ROOT_FIELDS;

async function mergeSchemaExtends(type, schemaNameToType, configuredSchemaGetterByType) {
  ...
  const mergeSchemaExtends = async (subsShema, parentSchema) => {
    ...
    deepextends(subsShema, { ...parentSchema,
      extends: undefined
    });
  };
  ...
  const mergeNextNamedSchema = async (subsShema, name) => {
    if (foundSchemaName[name]) {
      ...
      return; // throw new Error("bad schema configuration, circle reference found:" + name)
    }
  ...
  let resultSchema = {}; // starting from initial schema

  await mergeSchemaExtends(resultSchema, schema); // // await dynamic
  ...
  resultSchema = inflateSchema(resultSchema); // console.log("merged schema")
...
function inflateSchema(mergedSchema) {
  const fields = mergedSchema.fields; // merge audit
  ...
  for (let name in persistence) {
    ...
    if (info) {
      // normalize
      info.fieldMap = normalizeFieldMap(fields, info.fieldMap); // build reversed map
...
function mergeLaplaceFields(fields, laplaceFields) {
  if (!laplaceFields) {
    return [];
  } // extended and copied


  laplaceFields = deepcopy(deepextends(laplaceFields, fields)); // define the 'field' property for list
