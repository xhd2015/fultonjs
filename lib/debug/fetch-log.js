...
exports.LogFetcher = void 0;
exports.defaultDiagnoseMapGetter = defaultDiagnoseMapGetter;
exports.genlog = genlog;
exports.genlogStat = genlogStat;
exports.levelStrMap = exports.levelMap = void 0;
exports.renderStatToHTML = renderStatToHTML;
...
exports.levelStrMap = levelStrMap;

class LogFetcher {
  constructor(client) {
    this.client = client || _axios.default.request;
  }
  ...
  async genlog(psmList, options) {
    ...
    let psmMatch = psmList.map(PSM => ({
      match_phrase: {
        PSM
      }
    }));
    let psmMatchParam = JSON.stringify(psmMatch);
    let param = createReqeustParam({
      limit,
      beginTime: startTime,
      endTime: endTime,
      PSMMatchParam: psmMatchParam
    });
    let data = await this.client({
      ...
      method: "POST" // validateStatus: undefined,
...
let defaultFetcher = new LogFetcher(); // options: start,end,limit

async function genlog(psmList, options) {
  return await defaultFetcher.genlog(psmList, options);
}

async function genlogStat(psmList, options) {
...
function getRespStatistics(responses) {
  let locationIdx = 0;
  let psmUnknownIdx = 0; // PSM -> location -> message
...
let generalRules = [networkErrorRule, {
  ...
  "action": "ignore",
  "reason": "网络错误:连接断开"
}, bizServiceUnavailable, seriviceUnavailableRule, {
...
function defaultDiagnoseMapGetter(psmList) {
  let diagnoseMap = createDiagnoseMap(); // initialize
  ...
  for (let k in diagnoseMap) {
    let v = diagnoseMap[k];
    generalRules.forEach(rule => v.push(rule));
...
function renderStatToHTML(logStat, options) {
  let {
    showCount = 1,
    getDiagnose
  } = options || {};
...
`;
  ...
  for (let psm in errors) {
    let psmStat = errors[psm]; // sort by count in decreasing order

    let psmCount = psmStat["count"];
    let maps = psmStat["data"]; // { location : { count ,data:[{log}] } }

    let mapsArray = Object.keys(maps).map(k => [k, maps[k]]); // sort by count in descende order
