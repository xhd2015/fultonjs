...
function set(o, path, value, options) {
  path = toSegments(path);
  ...
  let last;
  let endNode = o; // locating its endNode, where endNode is the last node that
...
function del(o, path, options) {
  return set(o, path, DEL, options);
}

function assign(dst, src, options) {
  for (let key in src) {
