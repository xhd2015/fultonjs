...
exports.Mocker = exports.ANNOTATION = void 0;
exports.enumComment = enumComment;
...
function mockResponseMethods(service, mocks) {
  return mockMethodReturnType(service, 1
  /* response */
  , mocks);
}
/**
...
function mockRequestMethods(service, mocks) {
  return mockMethodReturnType(service, 0
  /* request */
  , mocks);
}
...
function mockMethodReturnType(service, type, mocks) {
  ...
  return Object.keys(serviceMethods).reduce((methods, method) => {
    ...
    const methodMessageType = type === 0
    /* request */
    ? serviceMethod.requestType : serviceMethod.responseType;
...
function mockField(field, stackDepth) {
  ...
  if (field.resolvedType instanceof _protobufjs.Type) {
...
function interpretMockViaFieldName(fieldName) {
  ...
  if (fieldNameLower.startsWith('id') || fieldNameLower.endsWith('id')) {
    return "id"; //   return uuid.v4();
  }

  return 'Hello';
}

function enumComment(enumType) {
  return enumType.name + ": " + Object.keys(enumType.values).map(key => [key, enumType.values[key]]).map(([k, v]) => `${v}=${k}`).join(" ");
...
class Mocker {
  ...
  mockTypeData(type) {
    ...
    if (type instanceof _protobufjs.Type) {
      if (type.oneofs) {
    ...
    if (type instanceof _protobufjs.Enum) {
      return {
        data: this.mockEnum(type),
        annotation: {
          [this.annotationKey]: enumComment(type)
        }
      };
    } else if (typeof type === 'string') {
      return {
        data: this.mockScalar(type),
        annotation: null
      };
    }
  ...
  get annotationKey() {
    ...
    return ((_a = this.options) === null || _a === void 0 ? void 0 : _a.annotationKey) || ANNOTATION;
  }
  /**
   * Mock methods request
   */
  ...
  mockRequestMethods(service) {
    return this.mockMethodReturnType(service, 0
    /* request */
    );
  }
  ...
  */
  ...
  _mockTypeFields(type) {
    ...
    const data = {};
    const annotation = {};
    return type.fieldsArray.reduce(({
      data,
      annotation
    }, field) => {
      field = field.resolve();
  ...
  _mockField(field) {
    ...
    if (field instanceof _protobufjs.MapField) {
      let {
        data,
        annotation
      } = this.mockTypeData(field.resolvedType || field.type);
  ...
  _pickOneOf(oneofs) {
    ...
    return oneofs.reduce(({
      ...
      return {
        ...
        annotation: Object.assign(Object.assign({}, annotation), {
