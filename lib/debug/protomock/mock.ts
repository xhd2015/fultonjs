...
export class Mocker {
    ...
    mockTypeData(type: string | Type | Enum): { data: any, annotation: any } {
        ...
        if (type instanceof Enum) {
            return { data: this.mockEnum(type), annotation: { [this.annotationKey]: enumComment(type) } }
    ...
    get annotationKey(): string | symbol {
        return this.options?.annotationKey || ANNOTATION
    }
    ...
    */
    ...
    _mockField(field: Field): MockResult {
        field = field.resolve()
        if (field instanceof MapField) {
            let { data, annotation } = this.mockTypeData(field.resolvedType || field.type);
