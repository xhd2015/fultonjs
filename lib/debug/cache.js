...
class AsyncCacher {
  ...
  constructor(loader, ttl, onCacheLoaded, options) {
    // old signature: (loader,ttl,onCacheLoaded)
    ...
    if (!entry) {
      ...
      entry.initResolver = this._loadOne(args).then(async v => {
        ...
        if (!this._shouldCacheNull && (v === null || v === undefined)) {
          this._deleteKey(key); // return, but do not cache
        ...
        this._checkEvict(); // console.log("after loaded,this.cache:", this.cache)
