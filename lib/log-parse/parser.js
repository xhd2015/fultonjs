...
// const isDebug = false;
const debug = (0, _debugUtil.makeDebug)(false);
...
class Parser {
      ...
      switch (s[j]) {
        ...
        case '\\':
          if (j + 1 < n && s[j + 1] === '\\') {
            ...
            pushChar('"');
            j++; // skip next
          } else if (j + 3 < n && (0, _char.isDigit)(s[j + 1]) && (0, _char.isDigit)(s[j + 2]) && (0, _char.isDigit)(s[j + 3])) {
  ...
  parseField(s, n, i) {
    ...
    let {
      ...
      value: fieldValue,
      next: fieldNext
    } = this.parseFieldValue(s, n, next + 1); // parseField should consume the extra separator if present.
    // especially ' '
...
function guess(s) {
  const opts = (0, _guess.guessLogOptions)(s);
