...
Object.keys(DEFAULT_VALUE_MAP).forEach(key => DEFAULT_VALUE_MAP_WITH_COMMENT[key] = { value: DEFAULT_VALUE_MAP[key], comment: key })
// schema keys: 
// [
//   "constants",
//   "enums",
...
class Schema {
    ...
    genServiceFunctionsDefault(service, options) {
        ...
        for (let func of foundService.functions || []) {
            ...
            types.push({ name: func.name, returnType, arguments: args })
        }
        return types
    }
    ...
    _genTypeDefault(valueMap, type, indent, options) {
        ...
        if ((foundType = this.structMapping[name])) {
            ...
            return valueMap[type] = { value, comment }
