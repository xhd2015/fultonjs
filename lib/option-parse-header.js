...
function parse(help, opts, argv) {
  ...
  for (let opt of optList) {
    ...
    if (optionRepeat[mapName] != null) {
      throw `duplicate option name:${mapName}`;
    }
    ...
    for (const optName of optNames) {
      ...
      if (optionNameMap[optName] != null) {
        throw `duplicate option name:${optName}`;
      }
  ...
  const args = [];
  const options = {};
  const detectArgForMe = {}; // used to iterate argv

  let i = 0;

  function emitArg(optName, arg) {
    let detect = arg === detectArgForMe; // clear arg if detect, avoid subtle bug
    ...
    if (optionRepeat[mapName] == 'zero') {
      ...
      if (arg != null) {
        if (arg == "on" || arg == "On" || arg == "ON" || arg == "true" || arg == "True" || arg == "TRUE") {
          ...
          throw `option requires no argument:${optName},except on/On/ON/true/True/TRUE or off/Off/OFF/false/False/FALSE`;
        }
      } else {
        options[mapName] = true;
      }
      ...
      if (arg == null) {
        throw `option requires one argument:${optName}`;
      }
      ...
      if (arg == null) {
        throw `option requires argument:${optName}`;
      }
      ...
      throw `unknown option repeat:${optName} ${optionRepeat[mapName]}`;
    }
  } // now, parse the argv
  ...
  for (; i < argv.length; i++) {
    ...
    if (arg.startsWith('--')) {
      ...
      if (optName.length == 1) {
        emitArg(optName, detectArgForMe);
        continue;
      } // many short options
      ...
      if (optReapt == 'one' || optReapt == 'many') {
        ...
        emitArg(optName[optName.length - 1], detectArgForMe);
      }
    } else {
      args.push(arg);
    }
