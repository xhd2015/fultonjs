...
export interface ORMOptions {
    ...
    // _00001
    // partTableCount?:number
    // partTableSuffixLen?:number
}
...
export class ORM {
    ...
    table: string
    options: ORMOptions
    constructor(db: any, options?: ORMOptions) {
        this.db = db
    ...
    normalizeModel(model, keepNull?: boolean): any {
        if (model === undefined) {
            return undefined
        }
        let checkedModel = {}
    ...
    modelToConditions(model, keepNull?: boolean) {
        let conditions = []
        let args = []
        for (let key in model) {
            let value = model[key]
    ...
    async find(where, options) {
        ...
        return await this.db.query("SELECT " + sqlFields + " FROM ?? WHERE " + conditionClause + orderBy + limitClause, [this.table, ...args]);
    }
