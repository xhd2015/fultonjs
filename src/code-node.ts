...
// example: ({"a":10 //\n}) => ({\n  "a": 10 //\n\n});
export function formatCode(code, options?: FormatCodeOptions) {
    const ast = parseAst(code)
    const { code: genCode } = generate(ast, { filename: 'code.js', ...options }, code)
    return genCode
}
...
function traverseAst(ast: any, f, after?: (ast: any) => any) {
    if (ast == null) return
...
function addTrailingComment(path, comment) {
    const commentOpt = {
        type: "trailing", // leading, inner or trailing
        line: true, // true => "CommentLine" , false => "CommentBlock",
        comment,
    }
