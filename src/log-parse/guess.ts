import { isDigit, isLetter, isWord } from "../char"
import { makeDebug } from "../debug-util"
import { ParseOptions, ParseOptionsV2, ServiceCalledRequestOptions, Parser } from "./parser"

const debug = makeDebug(true)

function guessOptionByColon(s: string) {

}

// some word, like: 'req:...',  we may consider after 'req:' is our complete log
function guessByLookingIdentifyingWord(s: string) {

}


// guess the log by looking ahead some key information
export function guessLogOptions(s: string): ParseOptionsV2[] {
    if (!s) {
        return []
    }
    const opts: ParseOptionsV2[] = []

    // first, try Service Called Request
    const reqOptions = tryParseReqOption(s)
    // if reqOptions present, then we think this is a strong hint
...
export function tryParseReqOption(s: string): ServiceCalledRequestOptions {
    if (!s) {
        return null
    }
...
//  3. count how many 
export function guessOptions(s: string): ParseOptions[] {
    if (!s) {
        return []
    }
    const opts = []
...
type LookingPrefixAction = string | {
    prefix: string
    autoObjectBrace?: boolean // auto add {} if missing
    options?: ParseOptions
}

interface ExtractOptions {
    lookingPrefix?: LookingPrefixAction[]
    autoLookingPrefix?: boolean // default false
    wantObject?: boolean // default:true
}

// try to parse all possibly object
export function tryExtractAll(s: string, opts?: ExtractOptions): any[] {
    const details = tryExtractAllWithDetail(s)
    return details.map(e => e.object)
}

export interface ExtractDetailItem {
    description?: string
    text: string
    object: any
    type: string
...
export function tryExtractAllWithDetail(s: string, opts?: ExtractOptions): ExtractDetailItem[] {
    if (!s) {
        return []
    }
    const items: ExtractDetailItem[] = []
    const wantObject = opts?.wantObject !== false
    if (opts?.lookingPrefix?.length > 0) {
        for (let opt of opts.lookingPrefix) {
            ...
            if (i >= 0) {
                ...
                if (parseOptions) {
                    try {
                        const parsed = new Parser(parseOptions).parse(usedPart)
                        if (parsed) {
                            items.push({
                                text: part,
                                object: parsed.value,
...
export function tryExtractByLookingPrefix(s: string): ExtractDetailItem[] {
    return []
}

// this strategy simple locate to some prefix, and do a strict parse
export function tryExtractByStartingFromKV(s: string): ExtractDetailItem[] {
    debug('tryExtractByStartingFromKV:', s)
    // look for `:"`, means it is a key value pair
    const n = s.length
    let col = 0
    ...
    debug("col effective:", { kvSep, stringQuoted, fieldSep })
    ...
    //   }
    debug("stats:", {
        col, colDoubleQuote, colDigit, colWord, doubleQuoteSpace, doubleQuoteComma,
        digitSpace, digitComma,
        wordSpace, wordComma,
    })
    return items
}