...
type Category = 'Service Called Reqeust' | ''

type Type = 'string' | 'number' | 'object' | 'boolean' | 'field' | 'array' | 'null'
...
export interface ParseOptions {
    ...
    stringWithQuote?: boolean | 'possible',
    arrayBeginChar?: string
    arrayEndChar?: string
    objectBeginChar?: string
    objectEndChar?: string
    // based on the category, the object 
    // singleObjectStart
}

export interface ParseOptionsV2 {
    category: Category
    serviceCalledRequestOptions?: ServiceCalledRequestOptions
}

export interface ServiceCalledRequestOptions {
    reqObjectStart: number // index of req:
}
...
interface ParseResult {
    type: Type
    value: string | number | object | Array<any> | Field
    next: number
}
...
export class Parser {
    ...
    // @private
    parseNext(s: string, n: number, i: number): ParseResult {
        for (let j = i; j < n; j++) {
            switch (s[j]) {
                case '"':
                case '\'':
    ...
    parseString(s: string, n: number, i: number): ParseResult {
        const b = s[i];
        if (b !== '"' && b !== '\'') {
    ...
    parseObject(s: string, n: number, i: number): ParseResult {
        if (s[i] !== '{') {
            throw new Error("not an object");
        }
        return this.parseObjectUntil(s, n, i + 1, '}')
    }
    parseObjectUntil(s: string, n: number, i: number, untilChar: string): ParseResult {
        const obj = {};
        let j = i;
        // empty
    ...
    parseField(s: string, n: number, i: number): ParseResult {
    ...
    parseFieldKey(s: string, n: number, i: number): ParseResult {
        let j = i;
        let key = '';
        for (; j < n; j++) {
            switch (s[j]) {
    ...
    parseFieldValue(s: string, n: number, i: number): ParseResult {
        return this.parseNext(s, n, i);
    }
    parseNumber(s: string, n: number, i: number): ParseResult {
        let num = 0;
        let j = i;
        for (; j < n; j++) {
    ...
    parseArray(s: string, n: number, i: number): ParseResult {
        ...
        for (; j < n; j++) {
            ...
            const field = fieldValue as Field
            // value>
            if (s[next] === '>') {
...
export function guess(s: string): any {
...
export function tryParseAll(s:string):any[]{
