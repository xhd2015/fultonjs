...
function tryParseAll(s){
    ...
    for(let i=0;i<n;i++){
       ...
       try{
         const {type,value,next} = parseObject(s,n,i);
         i = next - 1;
         objs.push(value);
       }catch(e){
            // ignore
...
function parseObject(s,n,i){
    ...
    for(;j<n;j++){
        const {type,value,next} = parseField(s,n,j);
...
function parseField(s,n,i){
    const {type:keyType, value:keyValue,next} = parseFieldKey(s,n,i);    
    if(next >= n || s[next]!==':'){
        throw new Error(`invalid field,no separator:${s.slice(i,next+1)},s[${i},${next+1}]`);
    }
    let {type:valueType, value:fieldValue, next:fieldNext} = parseFieldValue(s,n,next+1);
...
//       }
      debug("before j:",j);
      const {value:field,next} = parseField(s,n,j);
